...
unsigned width = BASE_WIDTH; // Defined for external linkage
unsigned height = BASE_HEIGHT;
GLuint prog = 0; // Defined for external linkage
GLuint vbo = 0; // Defined for external linkage

static lua_State *L; // Lua state
static bool use_lua = false; // Flag to toggle Lua rendering
static retro_environment_t environ_cb; // Needed for system directory
...
static void compile_program(void)
{
   ...
   glDeleteShader(vert);
   glDeleteShader(frag);
   fprintf(stderr, "[libretro-test]: Program compiled, prog = %u\n", prog);
}
...
static void setup_vao(void)
{
   ...
   glUseProgram(0);
   fprintf(stderr, "[libretro-test]: VBO created, vbo = %u\n", vbo);
}
...
static void init_lua(void)
{
   if (!L)
   {
      L = luaL_newstate();
      luaL_openlibs(L);
      // Register the gl module
      extern int luaopen_gl(lua_State *); // Declare luaopen_gl from module_gl.c
      luaL_requiref(L, "gl", luaopen_gl, 1);
      lua_pop(L, 1);
      fprintf(stderr, "[libretro-test]: Registered gl module\n");
   }

   // Load the Lua script from system directory
...
{
   L = NULL; // Ensure Lua state is null initially
   init_lua();
}
...
void retro_get_system_info(struct retro_system_info *info)
{
   ...
   info->library_name = "Libretro Core GL Lua";
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   // Reload Lua script on core restart
   init_lua();
...
{
   // Optionally reload Lua script on reset
   init_lua();
}
